...
The ground under everything is painted, not tiled: `grass_paint.py [--map name] [season ...]` reads `data/grass.json` (dials, and per season a palette of three tones as dark, base, light shades plus the pack sheets to cut sprigs from), the map, and the region rectangles, paints the whole world once as rounded patches of the three tones with flagstones (`stone` mortar and shades, laid in staggered courses) wherever the map draws a path or a cobbled square, scatters sprigs over the grass and onto every edge, and crops one PNG per grassy region into `assets/grass/<region>_<season>.png`. The sprigs come from the purchased grass pack in `raw/grass_pack/` (`GrassN.png` sheets with matching `Grass ShadowN.png`): every sprig is cut out with the shadow that touches it, its blades split into dark and light by brightness and recoloured to the patch's shades. Along every patch edge, sprigs of the neighbouring tone reach across so the two bleed into each other. The mountain map is painted from the hand-drawn `mountain mockup.png` at the repo root by `panels.py`: the mockup's colours are kinds of ground, each 600×448 panel of the map grid gets its crop tidied into flat colour blocks as the layout reference (plus the finished panel above it, so joins agree) with the river illustration as the style, and `panels.py stitch` feathers the overlaps into `raw/panels/fangridge.png`, which the map's `.json` names as its `art`; `grass_paint.py --map fangridge` then cuts and palette-snaps it per season. `mountain.py` reads the same mockup into the map's cells and home anchors. Regions cut from the same painting meet without a seam. `scripts/world/world.gd` shows the season's image per region under the terrain layers. Change the look by editing dials, colours, or sheets and re-running; the previews under `previews/world*.png` are how the user judges it.
