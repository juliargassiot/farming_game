...
The ground under everything is painted, not tiled: `grass_paint.py [--map name] [season ...]` reads `data/grass.json` (dials, and per season a palette of three tones as dark, base, light shades plus the pack sheets to cut sprigs from), the map, and the region rectangles, paints the whole world once as rounded patches of the three tones with flagstones (`stone` mortar and shades, laid in staggered courses) wherever the map draws a path or a cobbled square, scatters sprigs over the grass and onto every edge, and crops one PNG per grassy region into `assets/grass/<region>_<season>.png`. The sprigs come from the purchased grass pack in `raw/grass_pack/` (`GrassN.png` sheets with matching `Grass ShadowN.png`): every sprig is cut out with the shadow that touches it, its blades split into dark and light by brightness and recoloured to the patch's shades. Along every patch edge, sprigs of the neighbouring tone reach across so the two bleed into each other. The mountain map is built by `mountain.py` from `data/mountain.json`: a height field stepped into terraces (`rock` shades, grass patches where the tone field is light). Below every contour that faces the viewer it lays the approved `cliff_face` pieces from `props.png`, stacked two high where the wall is tall and finished with a `cliff_end`, and leaves crumbled stretches to rubble. Boulders and crags are map cells (`o`, `O`, `A`, and 3×2 `X` footprints anchored as `boulder_large` buildings) drawn as props by the world scene: dense in rubble, thickening across the apron toward the first wall, a few on every ledge, a ring of crags round the peak. Trails wear in from the meadow as the `grass_dirt` tile's texture, with paw prints and steps where they climb a wall; the peak takes a `snow` dusting. The map's `.json` names a palette in `data/palette.json`, and `palette.py` snaps the finished painting and every prop marked with that palette onto it, so all the district's art shares one set of colours. Regions cut from the same painting meet without a seam. `scripts/world/world.gd` shows the season's image per region under the terrain layers. Change the look by editing dials, colours, or sheets and re-running; the previews under `previews/world*.png` are how the user judges it.
