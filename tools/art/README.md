...
Declare the terrain pair in `tiles.json`: prompts, `tile_size`, `seed`, PixelLab `params` (use `"mode": "pro"`; the standard mode gives flat grass and gridded soil), `edge_blend` (pixels faded toward the flat terrain colour at every tile edge, which is what hides seams), `soften_upper` (pulls the upper terrain toward its mean so furrows stay faint), `field_colour` and `field_flatten` (the untilled look, derived from the upper tiles so edges stay aligned), and per-season `lower` prompts, `lower_colour` (the flat plain tile and the shift applied to the textured tiles) and `variants`. With `"variant_mode": "sprite"` each variant prompt becomes a `variant_size` transparent sprite dropped at a seeded offset onto the flat lower colour; without it the prompt is inpainted into the centre of the textured tile, which keeps texture around it. To compare looks first, list alternatives under `candidates` (`label`, `lower`, `seed`, `params` overrides) and run `art.py tile-candidates <name> [--season s] [--only 1,3 | --redo]`: one generation per candidate into `raw/tiles/<name>-candidates/<n>`, shown on a numbered sheet (`previews/art/<name>-candidates.png`) under the farmhouse, an oak, the farmer, and a planted plot. Once the user picks a number, copy its prompt, seed, and params into the season and run `tile-design <name> --season s --candidate <n>`, which reuses those tiles instead of generating. Then `art.py tile-design <name> [--season s] [--variants-only]` (one generation per tileset and per variant; the preview stacks a sample field per season with a tilled patch inside it), approval as above, and `art.py tile-import <name>`, which writes `assets/tiles/<name>_<season>.png` and its `.tres`. Atlas rows 0-3 are the Wang tiles ordered by corner mask NW NE SW SE (lower ↔ untilled field, index 0 flat), rows 4-6 the variants (`variant_count` rolled and flipped copies of the textured tile faded to flat inside an irregular round mask of `variant_radius`, flowered first, then the same with flowers removed), rows 7-10 the same Wang shapes as tilled cutouts and rows 11-14 those cutouts tinted by `wet_tint`. `scripts/world/world.gd` draws the field, the tilled cutouts, and the wet cutouts each on a dual grid, swaps the season's TileSet, and scatters variants.
