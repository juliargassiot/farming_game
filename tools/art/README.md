...
Declare the terrain pair in `tiles.json`: prompts, `tile_size`, `seed`, PixelLab `params` (use `"mode": "pro"`; the standard mode gives flat grass and gridded soil), `edge_blend` (pixels faded toward the flat terrain colour at every tile edge, which is what hides seams) and `soften_upper` (pulls the upper terrain toward its mean so furrows stay faint). `seasons` overrides the lower prompt per season and lists `variants`: prompts painted into the centre of the plain lower tile with a round mask, seen inside a 3×3 tiling so they stay seamless. Then `art.py tile-design <name> [--season s] [--variants-only]` (one generation per tileset and per variant; the preview stacks a sample field per season), approval as above, and `art.py tile-import <name>`, which writes `assets/tiles/<name>_<season>.png` and its `.tres`: rows 0-3 are the Wang atlas ordered by corner mask NW NE SW SE with the upper terrain recoloured to `field_colour` (untilled ground) and index 0 replaced by flat `lower_colour`; row 4 holds the textured lower tile and the variants; row 5 the tilled tile in the original upper colours. `scripts/farm/farm.gd` draws the field on a dual grid, swaps the season's TileSet, scatters variants, and overlays tilled and watered cells.
