...
func _initialize() -> void:
	...
	if args.is_empty():
		printerr("usage: screenshot.sh scenes/<system>/<scene>.tscn [name] [x,y] [map] [arrive]")
		quit(2)
		return
	_capture(args[0], args[1] if args.size() > 1 else args[0].get_file().get_basename(), args[2] if args.size() > 2 else "", args[3] if args.size() > 3 else "", args.size() > 4)


func _capture(scene_path: String, name: String, at: String, map_name: String, arrive: bool) -> void:
	...
	if arrive and game != null:
		game.set("arriving", true)
