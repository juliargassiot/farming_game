...
`grass.json` drives the painted ground, grass and stone paths alike: `dials` shape the patches (size, how much of the ground is light or dark, how far edges bleed, how many sprigs) and the paths' stones (`stone_size`, grain, edge light and shade), `pack` says which sprigs to cut from the grass pack sheets, and `seasons` give each season's palette of three grass tones, its `stone` mortar and shades, and the sheets it draws from. Edit it and run `tools/art/grass_paint.py` to repaint `assets/grass/`; judge the result in the world previews.
