...
`grass.json` drives the painted ground, grass and gravel paths alike: `dials` shape the patches (size, how much of the ground is light or dark, how far edges bleed, how many sprigs), `pack` says which sprigs to cut from the grass pack sheets, and `seasons` give each season's palette of three tones and the sheets it draws from. Edit it and run `tools/art/grass_paint.py` to repaint `assets/grass/`; judge the result in the world previews.
